            ...
            <div v-if="upload_type != 'icon'" class="flex-row gap-40">
                <div class="left-content">
                    ...
                    <el-scrollbar height="490px">
                        <el-tree ref="treeRef" v-loading="tree_loading" :current-node-key="type_data[0]?.id || 'all'" class="filter-tree" :data="type_data" node-key="id" highlight-current :props="defaultProps" empty-text="无数据" default-expand-all :filter-node-method="filter_node" @node-click="tree_node_event">
    ...
    <template v-if="!isCustomDialog">
        <div class="flex-col h">
            <div v-if="model_value_upload.length > 0" class="flex-row flex-wrap gap-10 h">
                <div v-for="(item, index) in model_value_upload" :key="item.id" :class="'upload-btn upload-btn-style-' + styles + ' ' + (styles == 2 ? 'br-none' : '')" :style="'height:' + upload_size + ';width:' + upload_size + ';'" @click="open_upload_manage(index)">
                ...
                <div v-if="limit != model_value_upload.length" :class="'upload-btn upload-btn-style-' + styles" :style="'height:' + upload_size + ';width:' + upload_size + ';'" @click="open_upload_manage()">
                    <icon name="add" :size="Number(size) / 2 + ''" color="c"></icon>
                </div>
            </div>
            <template v-else>
                <div :class="'upload-btn upload-btn-style-' + styles" :style="'height:' + upload_size + ';width:' + upload_size + ';'" @click="open_upload_manage()">
...
// 分类权限控制
const attachment_category_operate = computed(() => ({
    is_add: '1',
    is_edit: '1',
    is_del: '1',
    ...(common_store.common.config.attachment_category_operate || {}),
}));
// 附件管理权限
const attachment_operate = computed(() => ({
    is_upload: '1',
    is_edit: '1',
    is_del: '1',
    is_move: '1',
    ...(common_store.common.config.attachment_operate || {}),
}));
...
watch(
    () => props.visibleDialog,
    (val) => {
        if (props.isCustomDialog) {
            dialog_visible.value = val;
        }
    },
    { immediate: true }
);
    ...
    if ((!upload_store.is_upload_api && upload_store.category.length === 0) || bool) {
        ...
        UploadAPI.getTree()
            .then((res) => {
                ...
                const category = Array.isArray(res?.data?.attachment_category) ? res.data.attachment_category : [];
                type_data.value = [all_tree, ...category];
                type_data_list.value = category;
                upload_store.set_category(type_data_list.value);
                tree_loading.value = false;
            })
            .catch(() => {
                upload_store.set_is_upload_api(false);
                type_data.value = [all_tree];
...
//#endregion 图标 ----------------------------------------------------------end
const emit = defineEmits(['update:icon', 'operation_end', 'update:visibleDialog']);
// 确认
const confirm_event = () => {
    ...
    if (view_list_value.value.length > 0) {
        ...
        if (icon_value.value) {
            UploadAPI.addIcon({ icon_class: icon_value.value }).catch(() => undefined);
        }
    }
    view_list_value.value = [];
    search_filter.value = '';
...
// 上传回显替换文件事件
const open_upload_manage = (index?: number) => {
    dialog_visible.value = true;
    is_replace.value = typeof index === 'number';
    replace_index.value = typeof index === 'number' ? index : -1;
};
// 上传回显删除事件
...
// 关闭上传弹窗回调
const close_upload_model = (data?: uploadList[]) => {
    if (Array.isArray(data) && data.length > 0) {
        upload_list.value = [...data, ...upload_list.value];
        if (props.limit == 1 && props.isCheckConfirm) {
            view_list_value.value = [data[0]];
            check_img_ids.value = String(data[0].id || '');
        }
        data_total.value += data.length;
        return;
    }
    get_attachment_list('1');
};
