...
import { toUploadListItem, uploadAsset } from '@/services/upload';
...
const uploaded_items = ref<uploadList[]>([]);
// 是否给二维码加模糊效果
...
const submit_event = async (formEl: FormInstance | undefined) => {
    ...
    const uploadType = props.type === 'img' ? 'image' : props.type === 'video' ? 'video' : 'file';

    for (const item of form.value.file) {
        ...
        try {
            const result = await uploadAsset(uploadType, item.file, {
                maxSize: props.fileSize,
                accept: props.exts,
            });
            item.progress = 100;
            item.status = 'success';
            uploaded_items.value.unshift(toUploadListItem(result, item.file));
...
// 关闭弹窗
const close_dialog = () => {
    const uploadedItems = [...uploaded_items.value];
    reset_data();
    emit('close', uploadedItems);
};
const reset_data = () => {
