                ...
                <div v-for="(item, index) in form.images_list" :key="index" class="card-background box-shadow-sm re">
                    <div class="flex-col align-c jc-c gap-20 w">
                        ...
                        <el-form-item label="图片链接" class="w mb-16" label-width="60">
                            <link-picker v-model="item.nexa_link" />
                        </el-form-item>
                    </div>
...
import { emptyNexaLink } from '@/types/nexa-link';
...
watchEffect(() => {
    ...
    key.value = get_math();
    if (Array.isArray(form.value.images_list)) {
        form.value.images_list.forEach((it: Record<string, unknown>) => {
            if (it.nexa_link == null || typeof it.nexa_link !== 'object') {
                it.nexa_link = emptyNexaLink();
            }
        });
    }
});
